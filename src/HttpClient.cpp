...
void CURL_MULTI_DEFAULT_SETTING(CURLM *handle) {
  ...
  curl_multi_setopt(handle, CURLMOPT_NETWORK_CHANGED,
                    CURLMNWC_CLEAR_CONNS | CURLMNWC_CLEAR_DNS);
#endif
...
HttpTransfer::HttpTransfer(const HttpRequest &request) : url(request.url) {
  ...
  if (request.conn_timeout_ms > 0)
    curl_easy_setopt(this->curlEasy, CURLOPT_CONNECTTIMEOUT_MS,
                     request.conn_timeout_ms);
  ...
  case HttpRequest::POST: {
    ...
    curl_easy_setopt(this->curlEasy, CURLOPT_POSTFIELDSIZE,
                     request.body.size());
    break;
  }
  default: {
    curl_easy_setopt(this->curlEasy, CURLOPT_CUSTOMREQUEST,
                     request.methodName.c_str());
    if (request.body.size()) {
      curl_easy_setopt(this->curlEasy, CURLOPT_POSTFIELDS,
                       request.body.c_str());
      curl_easy_setopt(this->curlEasy, CURLOPT_POSTFIELDSIZE,
                       request.body.size());
    }
  ...
  curl_easy_setopt(this->curlEasy, CURLOPT_WRITEFUNCTION,
                   HttpTransfer::body_cb);
  curl_easy_setopt(this->curlEasy, CURLOPT_WRITEDATA, &response.body);
  curl_easy_setopt(this->curlEasy, CURLOPT_HEADERFUNCTION,
                   HttpTransfer::header_cb);
...
const HttpResponse &HttpTransfer::getResponse() const { return this->response; }
...
void HttpTransfer::finalize_transfer() {
  curl_easy_getinfo(this->curlEasy, CURLINFO_RESPONSE_CODE,
                    &this->response.status);
  curl_easy_getinfo(this->curlEasy, CURLINFO_TOTAL_TIME,
                    &this->response.elapsed);
}
...
size_t HttpTransfer::body_cb(void *ptr, size_t size, size_t nmemb,
                             std::string *data) {
  data->append((char *)ptr, size * nmemb);
  return size * nmemb;
}

size_t HttpTransfer::header_cb(void *ptr, size_t size, size_t nmemb,
...
BoundedSemaphore::BoundedSemaphore(size_t initial_count, size_t max_count)
  ...
  assert(initial_count <= max_count &&
         "initial_count must lower than max_count");
}
...
HttpClient::TransferState::TransferState(
    std::shared_future<HttpResponse> &&future, CURL *curl)
...
HttpClient::TransferTask::TransferTask(HttpRequest r)
    : transfer(r),
      state(std::shared_ptr<TransferState>(new TransferState{
          this->promise.get_future().share(), this->transfer.curlEasy})) {};
...
HttpClient::HttpClient()
    : sema_(MAX_CONNECTION, MAX_CONNECTION), uplinkAvgSpeed(SPEED_TRACK_WINDOW),
      downlinkAvgSpeed(SPEED_TRACK_WINDOW) {
...
float HttpClient::uplinkSpeed() const { return this->uplinkAvgSpeed.mean(); }
  ...
  while (1) {
    ...
    do {
      ...
      if (msg && (msg->msg == CURLMSG_DONE)) {
        ...
        if (mit != this->curl2Task.end() && mit->second) {
          ...
          // Tag it completed
          it->state->state.store(TransferState::State::Completed,
    ...
    if (this->stop_.load()) [[unlikely]] {
      ...
      for (auto it = transfers.begin(); it != transfers.end(); ++it) {
        ...
        it->promise.set_exception(std::make_exception_ptr(std::runtime_error(
            "The HttpClient stopped while task in the pool.")));
      }
    ...
    // Handle cancel
    {
      ...
      for (CURL *curlEasy : cancels) {
        ...
        it->promise.set_exception(std::make_exception_ptr(
            std::runtime_error("The task is cancelled.")));
    ...
    for (auto &&task : reqs) {
      ...
      curl_multi_add_handle(this->multi_,
                            this->transfers.back().transfer.curlEasy);
    }
  }
}
