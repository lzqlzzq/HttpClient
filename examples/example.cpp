		...
		threads.emplace_back([&client, &responses, &requestBodies, i]() {
			try {
				...
				responses[i] = client.request(
					"https://httpbin.org/get?thread=" + std::to_string(i),
					"GET",
...

				{
					...
					std::cout << "  Elapsed: " << responses[i].elapsed << "s" << std::endl;
...
void testCancel() {
	...
	auto transferState = client.send_request(
		"https://httpbin.org/delay/10",
		"GET"
	);
	
	auto now1 = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
	std::cout << "[" << std::put_time(std::localtime(&now1), "%Y-%m-%d %H:%M:%S") << "] " << "Wait for 3 seconds." << std::endl;
	std::this_thread::sleep_for(std::chrono::seconds(3));
	transferState->cancel();
	auto now2 = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
	std::cout << "[" << std::put_time(std::localtime(&now2), "%Y-%m-%d %H:%M:%S") << "] " << "Transfer cancelled." << std::endl;
}
